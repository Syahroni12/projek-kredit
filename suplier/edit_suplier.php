...
$suplier = query("SELECT * from suplier WHERE id_suplier=$id_suplier")[0]; //pada akhira  qury menambahkan array index nol, di karenakan array yang doi hasilkan query merupakan arrya numerik yang indexnya 0


if (isset($_POST["ubah"])) { //cek apakah tombol tambah (dalam form bukan di halaman utama ) sudah di 
    //tekan atau belum

    if (ubah_suplier($_POST) > 0) {
        ...
        header("Location:../suplier/edit_suplier.php");;
    }
} 
?>
...
if (isset($_SESSION['gagal_diubah'])) {
    echo $_SESSION['gagal_diubah'];

    unset($_SESSION['gagal_diubah']); // Hapus session setelah digunakan agar pesan tidak muncul lagi
}
...
?>
  ...
  <input type="hidden" name="id_suplier" value="<?= $suplier['id_suplier']; ?>">
    <input type="text" class="form-control" id="nama"name="nama_suplier" placeholder="Masukkan nama Pengguna" value="<?= $suplier['nama_suplier']; ?>" required>
    </div>
  <div class="form-group">

  <textarea class="form-control" name="alamat" rows="4" cols="50"placeholder="alamat" value="<?= $suplier['alamat']; ?>"><?= $suplier['alamat']; ?></textarea>
    


  </div>
  <div class="form-group">
    <input type="number" class="form-control" id="no-telfon"name="no_telfon" placeholder ="Masukkan no telfon"required value="<?= $suplier['no_telfon']; ?>">
  ...
  <div class="form-group">
        ...
        <select class="form-control" id="jenis kelamin"name="jenis_kelamin">
            <option value="<?= $suplier['jenis_kelamin']; ?>"><?= $suplier['jenis_kelamin']; ?></option>
